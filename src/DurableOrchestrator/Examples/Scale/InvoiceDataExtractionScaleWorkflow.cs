...
using DurableOrchestrator.Examples.Invoices;

namespace DurableOrchestrator.Examples.Scale;

[ActivitySource]
public class InvoiceDataExtractionScaleWorkflow(OpenAISettings openAISettings) : BaseWorkflow(OrchestrationName), IDurableOrchestratorPrompts
{
    private const string OrchestrationName = nameof(InvoiceDataExtractionScaleWorkflow);
    private const string OrchestrationTriggerName = $"{OrchestrationName}_{nameof(HttpStart)}";
        ...
        [OrchestrationTrigger] TaskOrchestrationContext context)
    {
        ...
        // add try-catch block to handle the deserialization exception, in case of such exception, write the markdown content to the blob storage
        try{
            ...
            orchestrationResults.Add(nameof(JsonSerializer.Deserialize), $"{ex.Message}", LogLevel.Error);
            // save the markdown content to the blob storage - for further analysis
            await CallActivityAsync(
                context,
                nameof(BlobStorageActivities.WriteBufferToBlob),
                new BlobStorageRequest
                {
                    StorageAccountName = input.InvoiceTargetInfo!.StorageAccountName,
                    ContainerName = input.InvoiceTargetInfo!.ContainerName,
                    BlobName = $"{input.InvoiceTargetInfo!.BlobName}.md",
                    Buffer = markdownContent
                },
                span.Context);
            return orchestrationResults.Results;
        }
        ...
        FunctionContext executionContext)
    {
        ...
        var instanceId = await StartWorkflowAsync(
            starter,
            ExtractInput<InvoiceDataExtractionWorkflowRequest>(requestBody),
            span.Context);
